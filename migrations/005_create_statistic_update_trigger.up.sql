...
CREATE OR REPLACE FUNCTION trigger_update_team_statistics()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        IF NEW.status = 'finished'::game_status THEN
            PERFORM update_team_statistics_for_game(NEW.id);
        END IF;
    ELSIF TG_OP = 'UPDATE' THEN
        -- Game status changed to finished or winner changed
        IF (OLD.status != NEW.status AND NEW.status = 'finished'::game_status) OR
           (OLD.winner != NEW.winner AND NEW.status = 'finished'::game_status) OR
           (OLD.team1_score != NEW.team1_score AND NEW.status = 'finished'::game_status) OR
           (OLD.team2_score != NEW.team2_score AND NEW.status = 'finished'::game_status) OR
           (OLD.team1_info != NEW.team1_info AND NEW.status = 'finished'::game_status) OR
           (OLD.team2_info != NEW.team2_info AND NEW.status = 'finished'::game_status) THEN
            PERFORM update_team_statistics_for_game(NEW.id);
        END IF;
    ELSIF TG_OP = 'DELETE' THEN
        -- Game deleted, update statistics for both teams
        IF OLD.status = 'finished'::game_status THEN
            PERFORM update_team_statistics_for_game(OLD.id);
        END IF;
    END IF;
