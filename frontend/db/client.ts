...
function createPool(): Pool {
    ...
    const poolConfig: PoolConfig = {
        ...
        idleTimeoutMillis: Number(process.env.DB_POOL_IDLE_TIMEOUT || 20000),
...
if (!globalForDb.__dbPool) {
    ...
    pool.on('error', (err: any) => {
        if (err.code === 'XX000' || err.message?.includes('shutdown') || err.message?.includes('termination')) {
            console.warn('Database connection terminated (expected for managed databases):', err.message || err.code);
            return;
        }
