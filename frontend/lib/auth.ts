...
function getCachedAuthSignature(walletAddress: string): AuthSignature | null {
  try {
    ...
    if (
      authSignature.walletAddress.toLowerCase() !== walletAddress.toLowerCase()
    ) {
  ...
  try {
    ...
    if (!signature || !message || !walletAddress) {
      return {
        isValid: false,
        error:
          "Missing required parameters: signature, message, or wallet address",
      };
    }
    ...
    if (!message.includes("Authenticate with TactiBall")) {
      return {
        isValid: false,
        error:
          "Invalid message format: must be TactiBall authentication message",
      };
    }
    ...
    if (!timestampMatch) {
      return {
        isValid: false,
        error:
          "Message does not contain valid timestamp format (YYYY-MM-DD HH:mm)",
      };
    }
    ...
    try {
      ...
      console.log(
        `Time remaining: ${Math.floor(timeRemaining / (60 * 60 * 1000))}h ${Math.floor((timeRemaining % (60 * 60 * 1000)) / (60 * 1000))}m`,
      );
